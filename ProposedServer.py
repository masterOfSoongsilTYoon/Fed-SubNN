...
from CosineClusteredFedAvgServer import parameter_to_Ndarrays, cosine_distance_cal
import os
import pandas as pd
...
if __name__=="__main__":
    ...
    main_model = LSTMModel(3, 4, 1, 2)
    if args.pretrained is not None:
        main_model.load_state_dict(torch.load(args.pretrained, weights_only=True))
    history=fl.server.start_server(server_address="[::]:8084",strategy=ProposedServer(main_model=main_model, omega=args.omega ,evaluate_fn = fl_evaluate,inplace=False, min_fit_clients=4, min_available_clients=4, min_evaluate_clients=4), 
    ...
    # plt=pd.DataFrame(history.metrics_centralized)
    # plt.plot().figure.savefig(f"./Plot/{args.version}_metrics.png")
    # plt.to_csv(f"./Csv/{args.version}_metrics.csv", index=False)